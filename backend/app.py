...
def sql_search(name, skin):
    # query_sql = f"""SELECT Name, Brand, Ingredients FROM products WHERE LOWER( Name ) LIKE '%%{episode.lower()}%%' limit 10"""
    ...
    query_ingreds = mysql_engine.query_selector(query_sql)
    query_ingreds = [ingred for ingred in query_ingreds.fetchone()]
